...
`mkdir -p ~/srv/postgres` | Windows: `mkdir -p $HOME\srv\postgres`

3. Run docker image
`docker run --rm --name lil-postgres -e POSTGRES_PASSWORD=password -d -v $HOME/srv/postgres:/var/lib/postgresql/data -p 5432:5432 postgres` | Windows: `docker run --rm --name lil-postgres -e POSTGRES_PASSWORD=password -d -v $HOME\srv\postgres:/var/lib/postgresql/data -p 5432:5432 postgres`
